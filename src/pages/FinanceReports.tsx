...
async function exportFinanceReportPdf(data: FinanceReportDashboardData): Promise<void> {
  const meta = data.meta;
  const doc = new jsPDF({ orientation: "portrait", unit: "pt", format: "a4" });

  let y = await drawPdfLetterhead(doc, {
    departmentName: meta.institute_name || DEFAULT_DEPARTMENT_NAME,
    documentTitle: meta.report_title,
  });
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(100, 116, 139);
  doc.text(`Period: ${meta.date_from} to ${meta.date_to}`, 40, y);
  y += 14;
  doc.text(`Generated: ${new Date(meta.generated_at).toLocaleString()}`, 40, y);
  y += 14;
  doc.text(`Generated by: ${meta.generated_by}`, 40, y);
  y += 18;
...
const FinanceReports = () => {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      await exportFinanceReportPdf(data);
    } finally {
